...
class Subscription extends \Opencart\System\Engine\Model {
	...
	public function getHistories(int $subscription_id, int $start = 0, int $limit = 10): array {
		...
		$query = $this->db->query("SELECT 
			...
			`sh`.`notify`,
			`sh`.`modified_by`
		FROM `" . DB_PREFIX . "subscription_history` `sh` 
		LEFT JOIN `" . DB_PREFIX . "subscription_status` `ss` 
			ON `sh`.`subscription_status_id` = `ss`.`subscription_status_id` 
			AND `ss`.`language_id` = '" . (int)$this->config->get('config_language_id') . "' 
		WHERE `sh`.`subscription_id` = '" . (int)$subscription_id . "' 
